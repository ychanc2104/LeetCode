...
# bottom-up dp, TC:O(nlogn), SC:O(n)
def jobScheduling4(startTime, endTime, profit) -> int:
    ...
    for i in range(len(profit)-1, -1, -1):
        target = pairs[i][1]
        L, R = 0, n-1
        # find jump end, leftmost, find first valid starting time after current end time
        # 1,2,3,4,8 t=6 => return index 5(8)
        while L <= R:
            mid = L + (R - L)//2
        ...
        dp[i] = max(dp[i+1], dp[L] + pairs[i][2])
    print(dp)
    return dp[0]
...
res3 = jobScheduling3(startTime, endTime, profit)
res4 = jobScheduling4(startTime, endTime, profit)
