...
# Bellman-Ford, bfs + dp, TC:O(|E|K), SC:O(N)
def findCheapestPrice6(n: int, flights , src: int, dst: int, k: int) -> int:
    dp = [float("inf")] * n # dp[i]: cheapest flight to i
    dp[src] = 0
    for i in range(k+1):
        dp_prev = dp.copy()
        for s,d,p in flights:
            if dp_prev[s] + p < dp[d]:
                # update
                dp[d] = dp_prev[s] + p # use prev price to prevent exceed k stops
